...
SteamCommunity.prototype.getSteamGroup = function (id, callback) {
  if (typeof id !== 'string' && !Helpers.isSteamID(id)) {
    throw new Error(
      'id parameter should be a group URL string or a SteamID object',
    );
  }

  if (
    typeof id === 'object' &&
    (id.universe != SteamID.Universe.PUBLIC || id.type != SteamID.Type.CLAN)
  ) {
    throw new Error(
      'SteamID must stand for a clan account in the public universe',
    );
  }
  ...
  this.httpRequest(
    'https://steamcommunity.com/' +
      (typeof id === 'string' ? 'groups/' + id : 'gid/' + id.toString()) +
...
function CSteamGroup(community, groupData) {
  ...
  this.avatarHash =
    groupData.groupDetails[0].avatarIcon[0].match(/([0-9a-f]+)\.jpg$/)[1];
...
CSteamGroup.prototype.getMembers = function (addresses, callback) {
  ...
  this._community.getGroupMembers(
    this.steamID,
    callback,
    null,
    null,
...
CSteamGroup.prototype.postAnnouncement = function (
  headline,
  content,
  hidden,
  callback,
) {
  this._community.postGroupAnnouncement(
    this.steamID,
    headline,
    content,
    hidden,
...
CSteamGroup.prototype.editAnnouncement = function (
  annoucementID,
  headline,
  content,
  callback,
) {
  this._community.editGroupAnnouncement(
    this.steamID,
    annoucementID,
    headline,
    content,
...
CSteamGroup.prototype.deleteAnnouncement = function (annoucementID, callback) {
  this._community.deleteGroupAnnouncement(
    this.steamID,
    annoucementID,
    callback,
  );
...
CSteamGroup.prototype.scheduleEvent = function (
  name,
  type,
  description,
  time,
  ...
  this._community.scheduleGroupEvent(
    this.steamID,
    name,
    type,
    description,
...
CSteamGroup.prototype.editEvent = function (
  id,
  name,
  type,
  description,
  ...
  this._community.editGroupEvent(
    this.steamID,
    id,
    name,
    type,
 ...
 */
CSteamGroup.prototype.respondToJoinRequests = function (
  steamIDs,
  approve,
  callback,
) {
  this._community.respondToGroupJoinRequests(
    this.steamID,
    steamIDs,
    approve,
    callback,
...
CSteamGroup.prototype.respondToAllJoinRequests = function (approve, callback) {
  this._community.respondToAllGroupJoinRequests(
    this.steamID,
    approve,
    callback,
  );

...
SteamCommunity.prototype.setupProfile = function (callback) {
  this._myProfile('edit?welcomed=1', null, function (err, response) {
    if (!callback) {
      return;
    }
      ...
      switch (i) {
        ...
        case 'primaryGroup':
          if (typeof settings[i] === 'object' && settings[i].getSteamID64) {
            ...
            values.primary_group_steamid = new SteamID(
              settings[i],
            ).getSteamID64();
          }
...
SteamCommunity.prototype.profileSettings = function (settings, callback) {
  this._myProfile('edit/settings', null, (err, response, body) => {
    ...
    const privacy = existingSettings.Privacy.PrivacySettings;
    ...
    for (const i in settings) {
      ...
      switch (i) {
        ...
        case 'inventoryGifts':
          privacy.PrivacyInventoryGifts = settings[i]
            ? SteamCommunity.PrivacyState.Private
            : SteamCommunity.PrivacyState.Public;
          break;
        ...
        case 'playtime':
          privacy.PrivacyPlaytime = settings[i]
            ? SteamCommunity.PrivacyState.Private
            : SteamCommunity.PrivacyState.Public;
          break;
      ...
      function (err, response, body) {
        if (err || response.statusCode != 200) {
          if (callback) {
            callback(
              err
                ? new Error(err.message + ' downloading image')
                : new Error(
                    'HTTP error ' + response.statusCode + ' downloading image',
                  ),
            );
          }
  ...
  this._myProfile(
    ...
    (_err, res, body) => {
      try {
        ...
        const match = body.blotter_html.match(/id="userstatus_(\d+)_/);
        if (!match) {
          callback(new Error('Malformed response'));
          return;
        }

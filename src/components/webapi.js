...
SteamCommunity.prototype.getWebApiKey = function (unused, callback) {
  ...
  this.httpRequest(
    ...
    (err, response, body) => {
      ...
      if (
        body.match(
          /Your account requires (<a [^>]+>)?Steam Guard Mobile Authenticator/,
        )
      ) {
        return callback(
          new Error(
            'Steam Guard Mobile Authenticator required to create a Steam Web API key',
          ),
        );
      }
      ...
      switch (body.success) {
        ...
        case SteamCommunity.EResult.Pending: {
          ...
          if (options.identitySecret) {
            this.acceptConfirmationForObject(
              options.identitySecret,
              finalizeOptions.requestID,
              err => {
                if (err) {
                  callback(err);
                } else {
...
SteamCommunity.prototype.setMobileAppAccessToken = function (token) {
  if (!this.steamID) {
    throw new Error(
      'Log on to steamcommunity before setting a mobile app access token',
    );
  }

  const decodedToken = Helpers.decodeJwt(token);
  ...
  if (decodedToken.sub != this.steamID.getSteamID64()) {
    throw new Error(
      `Provided token belongs to account ${decodedToken.sub}, but we are logged into ${this.steamID.getSteamID64()}`,
    );
  }
...
SteamCommunity.prototype._verifyMobileAccessToken = function () {
  ...
  const decodedToken = Helpers.decodeJwt(this.mobileAccessToken);

  const isTokenInvalid =
    decodedToken.sub != this.steamID.getSteamID64() ||
